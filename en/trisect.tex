...
\]
Compute the length $2a=\overline{CD}$ from Pythagoras's Theorem in $\triangle DCF$:
\begin{eqnarray*}
...
\coordinate (O) at (0,0)
  node[left] {$O$}
  node[above right,xshift=14pt,yshift=14pt] {\sm{\theta/2}};
\coordinate (A) at (9cm,0);
\node[right] at (A) {$A$};
\coordinate (B) at (60:9);
\node[above] at (B) {$B$};
...
\draw[dashed] (O) --  node[fill=white,near start] {\sm{\theta/2}} node[fill=white,near end] {$1$} (E);
...
\end{tikzpicture}
\end{center}
\caption{Second approximate trisection}\label{f.trisect-second-approx}
\end{figure}
...
%&=&\frac{(a_1+b_1\sqrt{c})(a_2-b_2\sqrt{c})}{a_2^2-b_2^2c}\\
