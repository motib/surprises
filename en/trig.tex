...
\begin{proof}
Let $\overline{AB}$ be a diameter and let $\angle BAC=\alpha$. Let $D$ be any other point on the circle that forms a triangle $\triangle BDC$, one of whose sides is the chord $\overline{BC}$. Since equal chords subtend equal inscribed angles $\angle BDC=\alpha$. In the right triangle $\triangle ABC$:
\[
...
$\overline{AB}\cdot\overline{CD}\cdot\overline{PQ}=\overline{QC}\cdot\overline{BP}\cdot\overline{AD}$ which can be re-arranged to obtain Eq.~\ref{eq.menelaus}.
\end{proof}
