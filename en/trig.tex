...
\coordinate (A) at (0,0);
\coordinate (C) at (8,0);
\path[name path=ab] (A) -- +(50:5);
\path[name path=cb] (C) -- +(150:7);
...
\node[above] at (B) {$B$};
\node[right] at (C) {$C$};
\draw (B) -- node[right] {$h$} (B|-A) coordinate (H);
...
Heron's formula computes the area of a triangle from its three sides.\index{Triangle!Heron's formula}
...
\begin{proof}
A radius of a circle and a tangent that intersects the radius are perdendicular. Furthermore, the lengths of the line segments of two tangents from the same point to the circle are equal. Therefore (Fig.~\ref{f.inscribed}):
\[
\triangle AOB'\cong \triangle AOC',\quad
\triangle BOA'\cong\triangle BOC',\quad
\triangle COA'\cong \triangle COB'\,.
\]
This also shows that the \emph{incenter}\index{Triangle!incenter},\index{Triangle!inscribed circle} the center of the inscribed circle, is the common intersection of the three angle bisectors.
...
The area $\triangle ABC$ is the sum of the six triangles listed above. Since the height of all six triangles is $r$, the radius of the inscribed circle, we obtain:
\begin{subeqnarray}
...
\begin{proof}
Let $\overline{AB}$ be a diameter and let $\angle CAB=\alpha$. Let $D$ be any other point on the circle one of whose sides is the chord $\overline{BC}$. Since equal chords subtend equal inscribed angles $\angle BDC=\alpha$. In the right triangle $\triangle ABC$:
\[
...
$\angle ADC+\angle ABC=180^\circ$ since the two angles are right angles. From Thm.~\ref{thm.interior-angles-of-a-polygon} the sum of the interior angles of a quadrilateral is $360^\circ$, so $\angle DAB+\angle DCB=180^\circ$. 
\begin{figure}[t]
\begin{center}
...
\end{figure}
Assuming that the diameter of the circle is $1$ (otherwise, multiply everything by the length of the diameter), the sides of the quadrilateral are:
\[
...
By Ptolemy's Theorem\index{Ptolemy's theorem} (Thm.~\ref{thm.ptolemy}) the product of the diagonals of a quadrilateral circumscribed by a circle is equal to the sum of the products of opposite sides of the quadrilateral. Using that $\angle ADC$ and $\angle ABC$ are right angles we have:
%
\[
...
\draw[<->] (0,-.5) -- node[fill=white] {$c-x$} ($(D)+(0,-.5)$);
\draw[<->] ($(D)+(0,-.5)$) -- node[fill=white] {$x$} (6,-0.5);
\end{tikzpicture}
...
\draw (a) -- (c) -- (b) -- (a);
\path[name path=bc] (b) -- (c);
\node[left,xshift=-14pt,yshift=-2pt] at (a) {$\alpha$};
\node[below,xshift=-18pt,yshift=-8pt] at (a) {$\alpha$};
