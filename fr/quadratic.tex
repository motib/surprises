...
\]
Pour les équations du second degré, on peut trouver les points $P_1$ et $P_2$ comme  intersections du segment qui représente le coefficient $b$ et du cercle dont le diamètre est le segment qui relie le point de départ et le point d'arrivée des chemins (fig.~\ref{f.lill-circle}). Pour qu'un point du segment  $b$ soit une racine, la droite doit être réfléchie à $90^\circ$ de sorte que l'angle inscrit soit sous-tendu par un diamètre.
...
Au début, les valeurs réelles calculées par la formule traditionnelle pour $r_2$ sont plus précises ($r_2-r_{2v}$ est négatif) mais à partir de $b=100\,000$, le calcul basé sur l'équation~\ref{eq.viete-quad} est plus précis. Telles sont les surprises de l'analyse numérique.
