...
\noindent\textbf{Construcción:}
Sea $\theta=\angle AOB$ un ángulo arbitrario y sin pérdida de generalidad supongamos que $A,B$ están en una circunferencia unitaria cuyo centro es $O$. Construir una circunferencia de radio $1/3$ con centro $O$ y que $D$ sea su intersección con $\overline{OA}$. Bisecamos $\angle AOB$ y sea $C$ la intersección de la bisectriz con la circunferencia de radio $1/3$. Construir la cuerda $\overline{CD}$ y las cuerdas $\overline{AE}=\overline{ET}=\overline{CD}$. Como cuerdas iguales subtienden ángulos centrales iguales $\angle TOE=\angle EOA=\phi$ (Fig.~\ref{f.trisect-second-approx}).
...
\end{theorem}
\begin{proof}
Construye $\overline{BC}$ y denota los ángulos y segmentos de recta como se muestra en Fig.~\ref{f.trisect-neusis-2}.
$\triangle ABC$ y $\triangle BCD$ son triángulos isóceles: $\overline{AB} = \overline{BC}$ son radios de la misma circunferencia y $\overline{BC} = \overline{CD}$ por construcción mediante la neusis. Como la suma de los ángulos de un triángulo es igual a $180^\circ$ y la suma de los ángulos suplementarios también es igual a $180^\circ$, tenemos:
\begin{eqnarray*}
...
\end{tikzpicture}
\end{center}
\caption{La construcción de neusis para trisecar un ángulo (2)}\label{f.trisect-neusis-2}
\end{figure}
...
\end{tikzpicture}
\caption{Una curva cuadrática}\label{f.trisect-quad-curve}
\end{center}
\end{minipage}
\hfill
\begin{minipage}{.48\textwidth}
...
\draw[thick] plot (4.6*.637*\x,{12.2*.637*\x*cot(\x r)});
\end{tikzpicture}
\caption{Un compás de cuadratura}\label{f.trisect-quad-compass}
\end{center}
\end{minipage}
\end{figure}

Se puede construir una cuadratriz utilizando un compás de cuadratriz como se muestra en la Fig.~\ref{f.trisect-quad-compass}. Consiste en dos rectas (no marcadas) que se mueven como se ha descrito anteriormente. Una articulación les obliga a moverse juntos y traza la curva.
\begin{figure}[b]
\begin{center}
...
\node[left] at (Q) {$Q$};
\end{tikzpicture}
\caption{Los puntos de intersección de una recta y un círculo}\label{f.constructible-line-circle}
\end{minipage}
\end{figure}
...
\begin{theorem}
Si un monic\index{polinomio mónico} polinomio cúbico con coeficientes racionales:
