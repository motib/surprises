...
Consideremos algunos valores de $-b,r_1,r_2$ y sea $m_{12}$ la media de $r_1,r_2$:
\[
...
\end{tikzpicture}
\caption{La zona es $x^2+4(b/4)x=x^2+bx$}\label{f.khw-1}
\end{center}
\end{minipage}
\hfill
\begin{minipage}{.45\textwidth}
...
\end{tikzpicture}
\caption{La zona es $x^2+4(b/4)x+4(b/4)^2=x^2+bx+(b^2/4)$}\label{f.khw-2}
\end{center}
\end{minipage}
\end{figure}

No podemos construir el diagrama en la Fig.~\ref{f.khw-1} porque no sabemos lo que $x$ es, pero el área del cuadrado más grande en la Fig.~\ref{f.khw-2} es:
\[
x^2+bx+\frac{b^2}{4}=c+\frac{b^2}{4}\,,
\]
que sí conocemos ya que nos dan los coeficientes $b,c$. Construyendo el diagrama y borrando los cuadraditos cuyos lados son $(b/4)$--otra cantidad conocida--obtenemos el segmento de recta de longitud $x$.
...
El método de Lill se puede aplicar para resolver ecuaciones cuadráticas.\footnote{Esta sección asume que has leído sobre el método de Lill en Chap.~\ref{c.origami-cube}.}\index{¡Método de Lill!ecuaciones cuadráticas}. Como ejemplo usamos la Ec.~\ref{eq.quadratic-lill} que da las raíces de una ecuación cuadrática obtenida por factorización:
\[
...
\end{figure}
Comprueba que los ángulos son correctos:
\[
...
\end{tikzpicture}
\end{center}
\caption{Construir un círculo para encontrar las raíces}\label{f.lill-circle}
\end{figure}
...
Inicialmente, los valores reales calculados mediante la fórmula tradicional para $r_2$ son más exactos ($r_2-r_{2v}$ es negativo), pero a partir de $b=100000$, el cálculo basado en la Ec.~\ref{eq.viete-quad} es más exacto. Tales son las sorpresas del análisis numérico.
