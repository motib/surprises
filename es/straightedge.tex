...
\vertex{P};
\end{tikzpicture}
\caption{Construcción de una línea paralela a la línea dirigida dada}\label{f.se-parallel-other2}
\end{center}
\end{minipage}
\end{figure}
...
Construimos un diámetro desde $X$ hasta $O$ que corte al otro lado de la circunferencia en $X'$, y construimos análogamente el diámetro $\overline{YY'}$. Construimos la semirrecta desde $X'$ pasando por $Y'$ y denotamos con $B$ su intersección con $l$. Afirmamos que $M$ es la bisectriz de $\overline{AB}$ de modo que $\overline{AB}$ es un segmento dirigido y por lo tanto se puede construir una recta que pase por $P$ paralela a $l$ (Fig.~\ref{f.se-parallel-other3}).
...
\end{tikzpicture}
\end{center}
\caption{Utilizar la circunferencia fija para copiar el segmento de línea}\label{f.se-copy3}
\end{figure}

\section[Un segmento de línea como cociente de segmentos]
{Construcción de un segmento de línea como cociente de segmentos}\label{s.relative}

\begin{theorem}\label{thm.straight-relative}
Dados segmentos de longitudes $n, m, s$, es posible construir un segmento de longitud:
\[x=\displaystyle\frac{n}{m}s\,.\]
\end{theorem}

\begin{proof}
Elegimos puntos $A,B,C$ que no estén en la misma recta y construimos semirrectas $\overline{AB}, \overline{AC}$. Por el Teorema~\ref{thm.straight-direction} se pueden construir puntos $M,N,S$ tales que $\overline{AM}= m$, $\overline{AN} =n$, $\overline{AS}=s$. Por el Teorema~\ref{thm.straight-parallel} construir una recta que pase por $N$ paralela a $\overline{MS}$ que corte a $\overline{AC}$ en $X$ y etiquetar $\overline{AX}$ (Fig.~\ref{f.se-three2}). $\triangle MAS\sim\triangle NAX$ por ángulo-ángulo-ángulo entonces $\displaystyle\frac{m}{n}=\displaystyle\frac{s}{x}$ y $x=\displaystyle\frac{n}{m}s$.
\end{proof}
...
Dados segmentos de longitudes $a,b$, es posible construir un segmento de longitud $\sqrt{ab}$.
\end{theorem}
