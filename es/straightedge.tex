...
\begin{proof}
Elegir puntos $A,B,C$ que no estén en la misma recta y construir rayos $\overline{AB}, \overline{AC}$. Por Teorema~\ref{thm.straight-direction} se pueden construir puntos $M,N,S$ tales que $\overline{AM}= m$, $\overline{AN} =n$, $\overline{AS}=s$. Por Teorema~\ref{thm.straight-parallel} construir una recta que pase por $N$ paralela a $\overline{MS}$ que corte a $\overline{AC}$ en $X$ y etiquetar $\overline{AX}$ por $x$ (Fig.~\ref{f.se-three2}). $\triangle MAS\sim\triangle NAX$ por ángulo-ángulo-ángulo así $\displaystyle\frac{m}{n}=\displaystyle\frac{s}{x}$ y $x=\displaystyle\frac{n}{m}s$.
\end{proof}
