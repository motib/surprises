...
$\{a,b,c\}$
נקראת 
\textbf{שלשה פיתגורית \L{}}.\index{Pythagorean triples}
\end{definition}
...
\section{משפט רמזי}\label{s.ramsey}

נצבע בשני צבעים את הקשתות של 
$K_5$, 
הגרף השלם בעל 
$5$
צמתים
(איור~%
\ref{f.ramsey5}).
אין תת-גרפים חד-גוניים
$K_3$
(משולשים) בגרף. איור~%
\ref{f.ramsey6}
מציג צביעה אחת של 
$K_6$
וניתן לראות שיש משולשים חד-גוניים 
$\triangle ACE$
ו-%
$\triangle BDF$.
בסעיף זה נוכיח מקרה פשוט של משפט רמזי
על קיומן של תת-קבוצות עם תכונה מסוימת.
\begin{figure}[htb]
\selectlanguage{hebrew}
\subcaptionbox{צביעה של $K_5$ בשני צבעים \label{f.ramsey5}}%
[.45\textwidth]
{
\centering
\begin{tikzpicture}
...
\hspace{3em}
\subcaptionbox{צביעה של $K_6$ בשני צבעים \label{f.ramsey6}}
[.45\textwidth]
{
\centering
\begin{tikzpicture}
...
מחקרים נוספים שיפרו את החסמים, אבל נושא זה עדיין פתוח כי החסמים אינם הדוקים. למשל, הוכח ש-%
$43\leq R(5) \leq 48$
ו-%
...
$k=4$:
\begin{eqn}
\overline{P}(n,4)&=&1-{n \choose 4}\cdot 2^{-5}=\left.\left(32-{n \choose 4}\right)\right/32\\
...
\subsection{ שלשות שור }\index{Schur triples}
...
\subsection{שלשות פיתגוריות }\index{Pythagorean triples}
...
עבור קוראים שאינם מכירים בסיסי ספירה לא-עשרוניים, נסקור בקצרה את המושג. "המספר"
$12345$
הוא קיצור למספר:
\[
