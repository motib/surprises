...
\L{Poh-Shen Loh}
הציע שיטת למצוא פרתונות למשואות ריבועית המבוססת על היחס בין המקדמים של הפולינום הריבועי לבין שורשיו. סעיף% 
~\ref{s.traditional}
...
אלגברה והסימונים האלגבריים הם פיתוח חדשה יחסית. בתקופות קדומות יותר מתמטיקאים השתמשו כמעט אך ורק בגיאומטריה, ולכן מעניין לעיין בבנייה הגיאומטרית של
\L{al-Khwarizmi}
עבור הנוסחאה למציאת השורשים של משוואה ריבועית (סעיף%
~\ref{s.khwar}).
סעיף%
~\ref{s.cardano}
מציג בנייה של ש-%
\L{Cardano}
השתמש בה בפיתוח הנוסחאה למציאת השורשים של משוואה ממעלה שלוש.
...
איור%
~\ref{f.loh-roots1}
מראה את היחסים הללו. אם נבחר ערכים אחרים
$r_1,r_2=3,5$
עבורם
$r_1+r_2=8$, $m_{12}=4$ 
נשאר ללא שינוי, אבל
...
כאשר
$c$
הוא הקבוע בפולינום. אם נכפיל את שני ביטויים שמצאנו עבור
$r_1,r_2$,
נוכל לחשב את
$s$
ואחר כך את
...
לפולינומים מעלות גבוהות יותר. הנה דוגמה מעניינת עבור משוואה ממעלה ארבע
\L{(quartic)}
$x^4-10x^2-x+20=0$.
כמו עם משוואות ריבועיות קיימות נוסחאות לפתרון משוואות ממעלה שלוש וממעלה ארבע (אבל לא למעלות גבוהות יותר), אבל הנוסאות די מסובכות.

האם פולינום זה מתפרק לשני פולינומים ריבועיים עם מקדמים שלמים? אם כן, המקדמים של גורם ה-%
$x^3$
חייבים להיות 
...
f(x) = (x^2 - x - 5)\, (x^2 + x - 4)\,.
\]
מפתרון של שתי המשוואות הריבועיות הללו נקבל ארבעה פתרונות למשוואה ממעלה ארבע:
\[
...
עבור פולינום שרירותי עם מקדם ראשון אפס
$x^2+bx+c$,
הנוסחאות של
\L{Loh}
הן:
...
\end{eqn}
שהיא הנוסחה המסורתית למצוא שורשים של פולינום. עבור פולינום עם מקדם ראשון
$a$,
הציבו במשוואה ופשטו:
\begin{eqn}
%ax^2+bx+c&=&0\\
...
השלמת הריבוע פותחה במאה השמינית על ידי
...
\end{tikzpicture}
\caption{=$x^2+4(b/4)x$ $x^2+bx$\\$\quad$}\label{f.khw-1}
\end{subfigure}
\hspace{2em}
...
\section{הבנייה של
\L{\large Cardano}
לפתרון משוואה ממעלה שלוש}
\label{s.cardano}

הנוסחה לשורשים של משוואה ממעלה שלוש פורסמה לראשונה במאה ה-%
$16$
על ידי 
\L{Gerolamo Cardano}.
לא נפתח כאן את הנוסחה, אבל הרעיון הבסיסי מעניין כי הוא מבוסס על בנייה גיאומטרית בדומה לבנייה של 
\L{al-Khwarizmi}.
הבנייה מתקבלת בצורה פשוטה מאלגברה. נכפיל ונקבל:
...
כך שהנפח שלו 
$(a+b)^3$.
נפרק את הקוביה לחמישה חלקים. שניים הראשונים הם קוביות שצלעיהן  
$a$
ו-%
$b$
עם נפח
...
שלושת החלקים האחרים הם קופסאות (המונח הפורמלי הוא
\L{cuboid}),
כל אחת עם צלע באורך
$a+b$
...
\L{\large Lill}
והמעגל של
\L{\large Carlyle}}\label{s.lill-quadratic}
...
נתון משוואה ריבועית
$x^2-bx+c$
(שימו לב שסימן המינוס בגורם הליניארי), נבנה את הנקודות
$(0,1)$
ו-%
$(b,c)$,
והמעגל שקוטרו הוא הקו המחבר את שתי הנקודות (איור%
~\ref{f.carlyle-circle}).
נקודות החיתוך שלו עם ציר ה-%
$x$
(אם הן קיימות) הן השורשים של המשוואה.
...
\textbf{אנליזה נומרית}
היא תחום במתמטיקה ומדעי המחשב בו מפתחים שיטות חישוב מדוייקות ויעילות. האתגר המרכזי היא לטפל בסופיות של ערכים שנשמרים בזיכרון של המחשב. קל לבצע את החישוב:
\[0.12\times 0.14=0.0168\]
אבל כדי לחשב:
\[0.123456789\times 0.123456789\]
המחשב חייב לשמור שמונה עשרה ספרות, דרישה שאי-אפשר למלא אם תאי הזיכרון במחשב מסוגלים לשמור שש עשרה ספרות בלבד. שגיאה זו נקראת 
...
עם השליטה המודרנית של שיטות אלגבריות בגיאומטריה, חשוב לזכור שהמצב היה הפוך. כפי שרואים מהבניות של
\L{Al-Khwarizmi}
ו-%
\L{Cardano},
...
ו-%
\L{Carlyle}
פיתחו שיטות גיאומטריות לפתרון של משוואות ריבועיות. שיקולים של חישובים נומריים יפתיעו סטודנטים שלא נחשפו קודם לנושא.
