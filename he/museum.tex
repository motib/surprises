...
$1973$
\L{Victor Klee}
שאל כמה שומרים נחוצים כדי לראות את כל הקירות של מוזיאון. אם הקירות של המוזיאון מהווים מצולע משוכלל או אפילו מצולע קמור, אפשר להסתפק בשומר אחד (איור%
~\ref{f.museum.convex}).
\begin{figure}[tb]
\begin{center}
...
$15$
קירות. כל "שן" מגדירה משולש שמסומן באפור באיור%
...
~\ref{f.museum.shaded}).
מכאן שחמש שומרות מספיקות כדי לראות על כל הקירות. בגלל שהמשולשים לא נחתכים שומרת במשולש אחד לא יכולה לראות את כל הקירות של משולש אחר (חץ ירוק) ולכן חמש שומרות נחוצות.
\begin{figure}[tb]
\begin{center}
\begin{tikzpicture}[scale=1]
\coordinate (O) at (0,0);
...
$n/3$
שומרות מספיקות לשמור על כל מוזיאון.
...
המיפוי של קודקודים לצבעים הוא שרירותי, כך שאם הקודקודים 
$A,B$
...
(איור%
~\ref{f.museum.three-3}).
נהדביק את שני המצולעים כדי לשחזר את המצולע המקורי עם
$n$
קודקודים 
(איור%
