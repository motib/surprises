...
שלושה שורשים
$-1,-2,-3$.
מחישוב השלילה של הארק-טנגנס שהשורשים מתקבל:
\[
...
דוגמה זו מדגימה שיטה שהומצאה על ידי
\L{Eduard Lill}
ב-%
$1867$.
בשיטה הגרפית אפשר למצוא את השורשים הממששים של כל פולינום. למעשה השיטה לא מוצאת שורשים אלא מוודא שערך נתון הוא שורש.
...
$x^3-2$.
בבנייה של המסלול הראשון נפנה שמאלה שמאלה בלי לבנות קטעי קו, כי 
$a_2$
ו-%
$a_1$
שניהם אפס. נפנה שוב שמאלה (למטה) ונבנה קו לאחור (למעלה) כי 
$a_0=-2$
...
\end{center}
\selectlanguage{hebrew}
\caption{הוכחת השיטה של Lill}\label{f.lill-proof}
\end{figure}
...
\section{%
הקיפול של
\L{\large Beloch}}\label{s.beloch-fold}
