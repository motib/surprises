...
$(0,0)$
מסלול המורכב מארבעה קטעים באורכים שלהלן (איור%
~\ref{f.lill1}):
\[
\{a_3=1,a_2=6,a_1=11,a_0=6\}\,.
\]
הבנייה מתחילה מראשית הצירים בכיוון החיובי של ציר ה-%
$x$
תוך סיבוב של
$90^\circ$
בין הקטעים. בנו מסלול
שני המתחיל בקטע היוצא מראשית הצירים ויוצר זווית בת 
$63.4^\circ$
עם ציר ה-%
$x$,
וסמנו ב-%
...
פנו 
$90^\circ$
שמאלה פעם נוספת, בנו קטע ושימו לב שהוא חותך את קצה המסלול הראשון הנמצא ב-%
$(-10,0)$.
...
\draw[very thick] (A) --
  node[below,xshift=5pt,yshift=-8pt] {$a_3=1$} (B);
\draw[very thick,name path=bc] (B) -- 
  node[right,yshift=8pt] {$a_2=6$} (C);
\draw[very thick,name path=cd] (C) --
  node[above,xshift=8pt] {$a_1=11$}(D);
...
שלושה שורשים
$-1,-2,-3$.
מחישוב הנגדי של הארקטנגנס של השורשים מתקבל:
\[
\alpha=-\tan^{-1} -1 = 45^\circ,\quad \beta\approx -\tan^{-1}-2 = 63.4^\circ,\quad \gamma=-\tan^{-1} -3\approx 71.6^\circ\,.
\]
באיור~%
\ref{f.lill2}
רואים שעבור כל אחת מהזוויות, המסלול השני חותך את קצה המסלול הראשון.
...
שעבורה
$-\tan 56.3=-1.5$
אינו שורש, המסלול השני חותך את המשך הקטע 
$a_0$,
אבל לא ב-%
$(-10,0)$,
קצה המסלול הראשון.
...
דוגמה זו ממחישה שיטה שהומצאה על ידי אדוארד ליל
\L{(Eduard Lill)}
בשנת
$1867$.
בשיטה הגרפית אפשר למצוא את השורשים הממשיים של כל פולינום. למעשה השיטה אינה מוצאת שורשים אלא מוודאת שערך נתון הוא שורש.

סעיף%
~\ref{s.method}
מציג תיאור פורמלי של השיטה של ליל
(מוגבל לפולינומים ממעלה שלישית) ומביא דוגמאות עבור מקרים מיוחדים. הוכחה לנכונות השיטה של ליל
מובאת בסעיף%
~\ref{s.proof}.
סעיף%
~\ref{s.beloch-fold}
מראה איך ליישם את השיטה באמצעות הקיפול של בלוץ'
\L{(Beloch)},
שהוא למעשה האקסיומה השישית של האוריגמי, אבל קדם לפורמליזציה של האוריגמי בשנים רבות.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{הצגת השיטה של ליל
\L{ Lill}}\label{s.method}

\subsection{השיטה של ליל 
כאלגוריתם}
\begin{itemize}
\item 
...
$a_3,a_2,a_1,a_0$
(בסדר זה) נבנה קטע המתחיל בראשית הצירים
$O=(0,0)$
בכיוון החיובי של ציר ה-%
$x$.
נפנה 
$90^\circ$
נגד כיוון השעון בין הקטעים.
\item 
נבנה את המסלול השני:
\begin{itemize}
...
אם 
$R$
היא נקודת הקצה של המסלול הראשון, אז
$-\tan\theta$
הוא שורש של 
$p(x)$.
\end{itemize}
...
\begin{itemize}
\item 
בבניית המסלול הראשון, אם המקדם הוא שלילי, נבנה את הקטע 
\textbf{בכיוון ההפוך}.
\item
בבניית המסלול הראשון, אם המקדם הוא אפס, אין לבנות קטע אבל כן נבצע את הפנייה הבאה של
$\pm90^\circ$.
\end{itemize}
\item הערות:
\begin{itemize}
\item 
נקודת החיתוך של קטע עם
$a_i$"
היא חיתוך הקטע עם
$a_i$
או עם הקו המשכו.
...
$90^\circ$
כך שלמסלול השני תהיה נקודת חיתוך עם הקטעים במסלול הראשון או עם המשכו.
\end{itemize}
\end{itemize}
...
(סעיף%
~\ref{s.ax6})
מקדמים שליליים. נפעיל את השיטה של ליל
על הפולינום (איור~%
\ref{f.lill4}).\footnote{%
נדון בקווים המקווקווים העדינים בסעיף%
~\ref{s.noninteger}.}
נתחיל בבניית קטע באורך
$1$
לכיוון החיובי של ציר ה-%
$x$. 
אחר כך נפנה שמאלה
$90^\circ$
(למעלה), אבל המקדם שלילי ולכן נבנה קטע באורך 
$-3$,
זאת אומרת קטע באורך
$3$
\textbf{כלפי מטה},
הפוך מהכיוון שאנו פונים אליו. לאחר פנייה נוספת של
$90^\circ$
לשמאל, המקדם שוב שלילי כך שנבנה קטע באורך
$3$
לאחור, לכיוון ימין. לבסוף, נפנה עם הפנים כלפי מטה ונבנה קטע באורך
$1$.

המסלול השני מתחיל עם ישר בזווית
$45^\circ$
יחסית לציר ה-%
$x$.
נקודת החיתוך של הישר עם המשך הקטע 
$a_2$
היא
$(1,1)$.
נפנה 
$-90^\circ$
(ימינה), נבנה קטע שנקודת החיתוך שלו עם המשך הקטע  
$a_1$
היא
$(5,-3)$.
נפנה שוב
 $-90^\circ$,
נבנה קטע שנקודת החיתוך שלו היא בנקודת הקצה של המסלול הראשון, 
$(4,-4)$.

$-\tan 45^\circ=-1$,
ולכן 
$-1$:
הוא שורש ממשי של הפולינום:
...
\subsection{מקדמים שהם אפס}\label{s.zero}
...
ב-%
$x^3-7x-6=0$,
הוא אפס. עבור מקדם אפס, אנו "בונים" קטע באורך
$0$,
כלומר איננו משרטטים קו, אבל כן פונים
$\pm 90^\circ$
לפני ואחרי ש"בונים" אותו כפי שמסומן על ידי החץ הפונה למעלה בנקודה $(1,0)$ (איור%
~\ref{f.lill5}).
נפנה שוב ונבנה קטע באורך
$-7$,
כלומר באורך
$7$
לאחור אל
$(8,0)$. 
לבסוף, נפנה פעם אחת אחרונה ונבנה קטע באורך
$-6$
עד
$(8,6)$.
...
$x^2$
הוא אפס כך שלא נשרטט קטע ונפנה שמאלה. המקדם הבא הוא
$-2$
כך שהקטע הבא נבנה לאחור מ-%
$(1,0)$
ל-%
$(3,0)$.
לבסוף, המסלול פונה למטה וקטע באורך 
$1$
נבנה מ-%
$(3,0)$
ל-%
...
\]
לכן קיימים שני מסלולים נוספים: אחד שמתחיל ב-
...
\end{figure}
כדי להכפיל קובייה עלינו לחשב את
$\sqrt[3]{2}$,
שורש של הפולינום ממעלה שלישית
$x^3-2$.
בבנייה של המסלול הראשון נפנה פעמיים שמאלה בלי לבנות קטעים, כי 
$a_1$
ו-%
$a_2$
שניהם שווים לאפס. נפנה שוב שמאלה (למטה) ונבנה קטע לאחור (למעלה) כי 
$a_0=-2$
שלילי.  נבנה את הקטע הראשון של המסלול השני בזווית של
$-51.6^\circ$
מכיוון ש-%
...
\section{הוכחת השיטה של ליל}\label{s.proof}

נגביל את הדיון לפולינומים שהמקדם המוביל שלהם הוא אחד, 
$p(x)=x^3+a_2x^2+a_1x+a_0$,
אחרת אפשר לחלק ב-%
$a_3$
ולפולינום המתקבל אותם שורשים. באיור~%
\ref{f.lill-proof}
הקטעים במסלול הראשון מסומנים ב-%
$b_2,b_1,a_2-b_2,a_1-b_1$.
במשולש ישר-זווית אם אחת הזוויות החדות היא
$\theta$
השנייה היא
$90^\circ-\theta$.
מכאן שהזווית מעל 
...
\end{eqn}
נפשט את המשוואה האחרונה, נכפול ב-%
$-1$
ונכניס את ממקדמים של 
$-1$
לתוך החזקות:
\begin{eqn}
...
\end{center}
\selectlanguage{hebrew}
\caption{הוכחת השיטה של ליל}\label{f.lill-proof}
\end{figure}
...
גילתה קשר מרתק בין אוריגמי לשיטה של ליל:
יישום אחד בלבד של פעולה שנודעה בהמשך כאקסיומה%
~$6$
מייצרת שורש ממשי של כל פולינום ממעלה שלישית. לעתים מכנים את הפעולה "הקיפול של בלוץ'"

נדגים את השיטה על הפולינום
$p(x)=x^3+6x^2+11x+6$
(סעיף%
~\ref{s.magic}).
נזכור שקיפול הוא האנך האמצעי של הקטע המחבר בין כל נקודה לשיקוף שלה ביחס לישר הקיפול. אנו רוצים ש-%
$\overline{RS}$
באיור~%
\ref{f.beloch-fold1}
...
ו-%
$S$
הן זוויות ישרות כפי שמתחייב בשיטה של ליל.
\begin{figure}[tb]
\begin{center}
...
\end{center}
\selectlanguage{hebrew}
\caption{ הקיפול של בלוץ' עבור $x^3+6x^2+11x+6$}\label{f.beloch-fold1}
\end{figure}
...
איור%
~\ref{f.beloch-fold2}
מציג את הקיפול של בלוץ' 
עבור הפולינום
$x^3-3x^2-3x+1$
(סעיף%
~\ref{s.negative}).
$a_2$
הוא קטע על הישר האנכי 
$x=1$,
והקטע המקביל לו הוא
$a_2'$
על הישר
...
הדגמת שיטתו של ליל
כקסם תמיד מפתיעה. ניתן לבצע אותה באמצעות תוכנה גיאומטרית כגון גיאוגברה. מפתיע גם שהשיטה של ליל
שפורסמה בשנת
$1867$,
והקיפול של בלוץ'
שפורסם ב-%
$1936$
הקדימו בשנים רבות את אקסיומות האוריגמי.
