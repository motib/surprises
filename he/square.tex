...
מתמקד בחלק מאיור%
~\ref{f.square-kochansky}
שבו נוספו קטעי הקו המקווקווים. מכיוון שכל המעגלים הם מעגלי היחידה, אורכו של כל אחד מהם הוא 
$1$.
מכאן ש-%
$\overline{AOCD}$
הוא מעוין ולכן האלכסונים שלו ניצבים זה לזה וחוצים זה את זה בנקודה שסומנה
$K$,
$\overline{AK}=1/2$.
\begin{figure}[tb]
\begin{center}
...
\section{הבנייה הראשונה של רמנגו'אן
...
\end{center}
\selectlanguage{hebrew}
\caption{הבנייה עם ציון אורכי הקטעים}\label{f.ramanujan1a}
\end{figure}
...
&=&2^2+\left(\sqrt{2}-\disfrac{1}{3}\right)^2-2\cdot 2 \cdot \left(\sqrt{2}-\disfrac{1}{3}\right)\cdot \disfrac{\sqrt{2}}{2}=\disfrac{19}{9}\\
...
\end{tikzpicture}
\end{center}
\caption{הבנייה עם ציון אורכי הקטעים}\label{f.ramanujan2a}
\end{figure}
...
הקוואדרטריקס מתואר בסעיף%
~\ref{s.q}.

יהי
$t=\overline{DE}$
המרחק שעובר הסרגל האופקי כאשר הוא יורד לאורך ציר ה-%
$y$
ותהי 
$\theta$
הזווית שנוצרת בין הסרגל המסתובב ובין ציר ה-%
$x$.
יהי 
$P$
המיקום של הציר המחבר את שני הסרגלים. המקום הגיאומטרי של
$P$
הוא עקומת הקוואדרטריקס.
...
ויהי 
$G$
מקומו של הציר כאשר שני הסרגלים מגיעים לציר ה-%
$x$,
כלומר, 
$G$
היא נקודת החיתוך בין עקומת הקוואדרטריקס וציר ה-%
$x$
(איור%
~\ref{f.square-quad}).
\begin{figure}[tb]
...
\end{center}
\selectlanguage{hebrew}
\caption{לרבע את המעגל באמצעות קוואדרטריקס}\label{f.square-quad}
\end{figure}
