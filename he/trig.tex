...
\selectlanguage{hebrew}
\subcaptionbox{%
חישוב שטח משולש לפי בסיס וגובה
...
\begin{theorem}[(Heron) הרון]\label{thm.heron}
השטח של משולש 
$\triangle ABC$
ניתן על ידי:
\[
...
\begin{proof}
רדיוס המעגל ומשיק החותך את הרדיוס ניצבים זה לזה. בנוסף, אורכי הקטעים של שני משיקים למעגל היוצאים מאותה נקודה שווים. לכן
(איור%
...
מרכז המעגל החסום הוא נקודת החיתוך המשותפת לשלושת חוצי הזוויות.}
\[
...
\end{eqnlabels}
נגדיר עכשיו את הצלעות באמצעות הטנגנסים של הזוויות המרכזיות:
\begin{displaymath}
...
\subsection{הסינוס והקוסינוס של סכום ושל ההפרש של שתי זוויות} \label{s.sum-of-trig}
...
\end{tikzpicture}
\end{center}
\caption{איורים להוכחת הזהות לסינוס של סכום זוויות}\label{f.sin-sum1}
\end{figure}
...
\centering
\selectlanguage{hebrew}
\subcaptionbox{כל הזוויות הנשענות על אותו מיתר שוות
\label{f.chord-angle}}
[.45\textwidth]
{
\centering
...
\hspace{1em}
\subcaptionbox{מרובע חסום במעגל
...
ו-%
$D$
כל נקודה אחרת על המעגל היוצרת את המשולש
$\triangle BDC$.
בגלל שכל הזוויות הנשענות על אותו מיתר שוות,
$\angle BDC=\alpha$. 
במשולש ישר-זווית
$\triangle ABC$:
\[
...
אל
$\overline{AB}$
והשתמש בהגדרת הקוסינוס ובמשפט פיתגורס:
\begin{eqnlabels}
...
\end{tikzpicture}
\caption{הוכחה ראשונה של משפט הקוסינוסים}\label{f.law-cosines2}
\end{center}
\end{figure}
...
\begin{proof}(2)
ההוכחה השנייה משתמשת במשפט תלמי

(משפט~\ref{thm.ptolemy}).%
\footnote{%
סעיף 
~\ref{a.ptolemy}
השתמש במשפט הקוסינוסים כדי להוכיח את משפט תלמי. 
ההוכחה הראשונה מאפשרת הוכחה לא מעגלית. בנוסף, קיימות הוכחות למשפט תלמי
שאינן משתמשות במשפט הקוסינוסים.}
...
השווה ל-%
$\angle CAB$
והשוק שלה חותכת את המעגל ב-%
$C'$
ואז נבנה את הקטע 
$\overline{C'A}$.
זווית הנשענות על אותו מיתר שוות,
...
\end{tikzpicture}
\end{center}
\caption{הוכחה שנייה של משפט הקוסינוסים}\label{f.law-cosines3}
\end{figure}                       
...
\end{scope}
\end{tikzpicture}
\caption{מצולעים משוכללים בעלי 
$16$, $8$, $3$
צלעות חסומים במעגלים%
...
\section{משפט תלמי}\label{a.ptolemy}
\index{Ptolemy's theorem}
...
לפי שנוכיח את משפט תלמי
נוכיח משפטים על מרובעים וטרפזים.

\begin{theorem}\label{thm.quad-circum}
ניתן לחסום מרובע במעגל אם ורק אם הזוויות הנגדיות שלו משלימות (סכומן שווה ל-%
$180^\circ$).
...
\begin{proof}
(רק אם)
זווית היקפית במעגל שווה למחצית הקשת שעליה היא נשענת כך ש-%
$\angle DAB$
שווה לחצי הקשת 
$\widehat{DCB}$
ו-%
...
(איור~%
\ref{f.trap-1}).
שתי הקשתות ביחד משלימות להיקף המעגל, ולכן סכומן 
$360^\circ$. 
מכאן ש-%
$\angle DAB + \angle DCB = 180^\circ$
ו-%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{הוכחת משפט תלמי}

\begin{theorem}[\L{Ptolemy}]
נתון מרובע חסום במעגל, הנוסחה שלהלן מתארת את הקשר בין אורכי האלכסונים ואורכי הצלעות במרובע
(איור%
...
\end{tikzpicture}
\end{center}
\caption{משפט תלמי}\label{f.trig-ptolemy}
\end{figure}                       
...
\end{tikzpicture}
\end{center}
\caption{ משפט צ'בה}\label{f.ceva1}
\end{figure}

\begin{proof}
אם הגבהים של שני משולשים שווים, אז היחס בין שטחי המשולשים שווה ליחס בין הבסיסים שלהם. בשני התרשימים באיור%
~\ref{f.ceva2}
...
\]
נחסר את השטחים האפורים במשולשים ונקבל את היחס בין המשלושים האפורים באיור%
~\ref{f.ceva3}:
\[
...
\end{tikzpicture}
\end{center}
\caption{משולשים במשפט צ'בה}\label{f.ceva2}
\end{figure}
...
\end{tikzpicture}
\end{center}
\caption{החסרת שטחים במשפט צ'בהs}\label{f.ceva3}
\end{figure}
...
\]
כי הסדר של הקודקודים במשולש אינו משנה.
\end{proof}
...
\end{tikzpicture}
\end{center}
\caption{משפט מנלאוס}\label{f.menelaus}
\end{figure}
...
\textbf{ישר חותך}
\L{(transveral)}
החותך את שלוש צלעות המשולש או את המשכן
(איור.~\ref{f.menelaus}).
אזי:%
\footnote{%
כתלות במבנה המשולשים והקו החותך, המכפלה יכולה להיות
$+1$
או
$-1$.}
\begin{equation}
