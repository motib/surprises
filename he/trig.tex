...
השטח של
$\triangle ABC$
הוא סכום השטחים של ששת המשולשים הרשומים לעיל. הגובה של כל  אחד מהמשולשים הוא 
$r$,
...
עם זווית חדה
$\beta$,
ניתן לחבר אותם ולקבל מצולעים עם זווית 
$\alpha+\beta$ (איור.~\ref{f.sin-sum1}).
המצולע השמאלי מראה את המצולע המופיע לעתים קרובות בהוכחות בספרי לימוד. כאן נביא הוכחות המבוססות על המצולעים במרכז ובימין.
\begin{figure}[htb]
\begin{center}
...
\end{tikzpicture}
\end{center}
\caption{כל הזוויות הנתמכות על ידי מיתר שוות}\label{f.chord-angle}
\end{figure}
...
$\triangle BDC$.
בגלל שכל הזוויות הנתמכות על ידי אותו מיתר שוות,
$\angle BDC=\alpha$. 
במשולש ישר-זווית
$\triangle ABC$:
\[
...
\begin{proof}[2]
ההוכחה מבוססת על האיור הימני באיור%
~\ref{f.sin-sum1}
שנמצא גם ב-%
...
\end{tikzpicture}
\end{center}
\caption{מרובע חסום בתוך מעגל}\label{f.trig-quad-circle}
\end{figure}
...
\section{משפט \L{Ptolemy}}\label{a.ptolemy}
\index{Ptolemy's theorem}
...
ניתן לחסום מרובע על ידי מעגל אם ורק אם הזוויות הנגדיות משלימות (הסכום שווה ל-%
$180^\circ$).
...
\begin{proof}[\textbf{רק אם}]
זווית היקפית במעגל שווה למחצית הקשר שתומך בו כך ש-%
$\angle DAB$
שווה לחצי הקשת 
$\widehat{DCB}$
ו-%
$\angle DCB$
שווה לחצי הקשר
$\widehat{DAB}$ (\ref{f.trap-1}).
שתי הקשתות ביחד מקיפים את כל היקף המעגל וסכומן הוא
$360^\circ$. 
מכאן ש-%
$\angle DAB + \angle DCB = 180^\circ$
ו-%
...
\hspace{3em}
\subcaptionbox{%
הקודקוד הרביעי חייב להיות על היקף המעגל
\label{f.trap-2}}
[.45\textwidth]
{
\centering
...
נתון מרובע החסום על ידי מעגל, הנוסחה שלהלן מתאר את הקשר בין אורכי האלכסונים ואורכי הצלעות.
(איור%
...
\begin{proof}
אם הגבהים של שני משולשים שווה, אזי היחסים בין השטחים שווים ליחסים בין לבסיסים. בשתי התרשימים באיור%
~\ref{f.ceva2}
...
\section{המשפט של \L{Menelaus}}\label{a.menelaus}
...
$\overline{DBQ}$
\textbf{קו חותך}
החותך את שלושת הצלעות של המשולש או ההארכות שלהם.%
(איור.~\ref{f.menelaus}).
אזי:%
\footnote{%
